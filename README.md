...
`npm run worker:once` only plans this mode. It creates one immutable, idempotent `SCHEDULED` Publication per Offer version and Web channel with operational state `AWAITING_VISUAL_INSPECTION`, then defers execution. The worker never opens Playwright for a Web Publication. Telegram and each WhatsApp group are planned independently; a Telegram success does not consume the Web channel decision. Open `/publicacoes` to inspect the saved snapshot and copy the safe local inspection commands. Browser preparation and any explicitly confirmed delivery remain separate CLI operations.

Install Chromium explicitly with `npm run whatsapp:web:install-browser`, then use the local `login`, `health`, `locate`, `dry-run`, `preflight`, `inspect-layout`, `inspect-draft`, no-send `inspect-delivery`, no-browser `resolve-delivery` and sanitized `config-check` commands documented in [docs/whatsapp-groups.md](docs/whatsapp-groups.md). Caption validation resolves an active media surface from preview, media controls, stacking context, geometry and before/after contenteditable fingerprints; it rejects the normal composer and requires a visible, positive-area, topmost, focused caption containing the exact snapshot in two observations. Post-click confirmation uses hashed structural baselines and recognizes new or optimistically reused outgoing elements with matching URL/title/media and no pending/error signal; private message content is not persisted. Any failure after click initiation remains `DELIVERY_UNCERTAIN`, blocks automatic retry and can be reconciled auditably without resending. No real send is performed by tests.
