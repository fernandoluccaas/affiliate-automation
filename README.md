...
Install Chromium explicitly with `npm run whatsapp:web:install-browser`, then use the local `login`, `health`, `locate`, `dry-run`, `preflight`, `inspect-draft` and sanitized `config-check` commands documented in [docs/whatsapp-groups.md](docs/whatsapp-groups.md). Caption validation is scoped to the active media overlay and requires a visible, positive-area, topmost, focused contenteditable containing the exact snapshot in two observations. Preflight performs only a send-button trial click; `inspect-draft` holds the safe draft for explicit human review and never sends. `config-check` and publish validate all authorization, including the current human visual-inspection fingerprint, before acquiring media, profile lock or Chromium. Real send additionally requires `WHATSAPP_WEB_DRY_RUN=false`, a current dry-run fingerprint and `--confirm-send`; its start/completion timestamps are persisted before visual confirmation. Any failure after click initiation remains `DELIVERY_UNCERTAIN` and blocks automatic retry. No real send is performed by tests.
