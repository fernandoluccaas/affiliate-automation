...
Install Chromium explicitly with `npm run whatsapp:web:install-browser`, then use the local `login`, `health`, `locate`, `dry-run` and `preflight` commands documented in [docs/whatsapp-groups.md](docs/whatsapp-groups.md). Preflight repeats the exact group, media, caption, unique affiliate URL and media-editor send-trigger checks, then clears the draft without exposing any click method. Real send additionally requires `WHATSAPP_WEB_DRY_RUN=false`, a current dry-run fingerprint and `--confirm-send`; its start/completion timestamps are persisted before visual confirmation. Any failure after click initiation becomes `DELIVERY_UNCERTAIN` and blocks automatic retry. No real send is performed by tests.
