...
Install Chromium explicitly with `npm run whatsapp:web:install-browser`, then use the local `login`, `health`, `locate`, `dry-run`, `preflight` and sanitized `config-check` commands documented in [docs/whatsapp-groups.md](docs/whatsapp-groups.md). Preflight waits for a stable media editor, tolerates at most one caption-input recreation, verifies the complete snapshot and unique affiliate URL, repeats the media-editor send-trigger checks, then clears the draft without exposing any click method. `config-check` and publish validate all real-send authorization before acquiring media, profile lock or Chromium. Real send additionally requires `WHATSAPP_WEB_DRY_RUN=false`, a current dry-run fingerprint and `--confirm-send`; its start/completion timestamps are persisted before visual confirmation. Any failure after click initiation becomes `DELIVERY_UNCERTAIN` and blocks automatic retry. No real send is performed by tests.
