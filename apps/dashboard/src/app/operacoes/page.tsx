import { collectOperationalStatus, collectStateAudit } from "@affiliate/operations";
    ...
    <AdminShell currentPath="/operacoes" title="Operacoes locais">
      ...
      <section className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
        ...
        <StatusCard
          ...
          detail={`PID ${status.supervisor.pid ?? "-"} / ${status.supervisor.uptimeSeconds}s`}
        />
          ...
          {status.components.map((component) => (
            <div key={component.component} className="rounded border p-3 text-sm">
              ...
              <div className="mt-1 text-xs">
                PID {component.pid || "-"} / restarts {component.restartCount}
              </div>
              {component.action ? (
            ...
            <tbody>
              {status.whatsappQueues.map((queue) => (
                <tr key={queue.channelId} className="border-b">
                  <td className="px-3 py-2 font-mono text-xs">{queue.channelId}</td>
      ...
      <section className="rounded-md border bg-white p-4">
        ...
        <div className="mt-3 grid gap-2">
          {["npm run ops:preflight", "npm run ops:status", "npm run ops:audit-state", "npm run ops:backup-status"].map(
            (command) => (
