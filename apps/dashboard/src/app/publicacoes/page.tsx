...
import { WhatsAppPublicationStepper } from "@/components/whatsapp-publication-stepper";
...
export default async function PublicationsPage() {
  ...
  return (
    <AdminShell currentPath="/publicacoes" title="Publicações">
      ...
      {publications.length === 0 ? (
        <EmptyState
          ...
          description="Quando o worker planejar uma oferta pronta, ela aparecerá nesta lista com o estado e o canal de destino."
          actionHref="/ofertas"
          actionLabel="Ver ofertas"
        />
      ) : (
        <section className="rounded-md border bg-[var(--surface)]">
          <p className="border-b bg-[var(--info-subtle)] px-4 py-2 text-xs text-[var(--info)]">
            Tabela larga: use rolagem horizontal. As colunas de controle e
            revisão permanecem visíveis à direita.
          </p>
          <div
            className="overflow-x-auto focus-visible:outline focus-visible:outline-2 focus-visible:outline-offset-2"
            tabIndex={0}
            aria-label="Publicações com rolagem horizontal"
          >
            <table className="w-full min-w-[1540px] text-left text-sm">
              <thead className="border-b bg-[var(--muted)] text-xs uppercase text-[var(--muted-foreground)]">
                <tr>
                  ...
                  <th className="px-4 py-3">Preços publicados</th>
                  ...
                  <th className="sticky right-[300px] z-20 bg-[var(--muted)] px-4 py-3">
                    Controle Web
                  </th>
                  <th className="sticky right-0 z-20 bg-[var(--muted)] px-4 py-3">
                    Revisão
                  </th>
                </tr>
              </thead>
                  ...
                  return (
                    <tr key={publication.id} className="border-b last:border-0">
                      <td className="px-4 py-3">
                        ...
                        />
                        {webView ? (
                          <span className="mt-2 inline-flex rounded-full border border-amber-300 bg-[var(--warning-subtle)] px-2 py-1 text-[11px] font-semibold text-[var(--warning)]">
                            {webView.badge}
                          </span>
                        ) : null}
                      </td>
                      ...
                      <td className="sticky right-[300px] z-10 min-w-[360px] border-l bg-[var(--surface)] px-4 py-3 align-top">
                        {webView ? (
                          <details className="rounded border border-amber-200 bg-[var(--warning-subtle)]/50 p-3">
                            <summary className="cursor-pointer font-medium">
                              {webView.badge}
                            </summary>
                            <div className="mt-3 min-w-[720px]">
                              <WhatsAppPublicationStepper
                                currentStep={
                                  webView.deliveryConfirmedAt ||
                                  publication.status === "PUBLISHED"
                                    ? 5
                                    : webView.sendWasClicked
                                      ? 4
                                      : webView.preflightCompleted &&
                                          webView.authorizationStatus ===
                                            "ACTIVE"
                                        ? 3
                            ...
                            <dl className="mt-3 grid grid-cols-[140px_1fr] gap-x-2 gap-y-1 text-xs">
                              ...
                              <dt>Autorização</dt>
                              <dd>
                                {webView.authorizationStatus ?? "ausente"} /
                                expira{" "}
                                {formatDateTime(webView.authorizationExpiresAt)}{" "}
                                / fp{" "}
                                {webView.authorizationFingerprint?.slice(
                                  0,
                                  12,
                                ) ?? "-"}
                              </dd>
                              ...
                              <dt>Autorização consumida</dt>
                              <dd>
                                {formatDateTime(
                                  webView.authorizationConsumedAt,
                                )}
                              </dd>
                              <dt>Marcador de clique</dt>
                              <dd>
                                inicio{" "}
                                {formatDateTime(webView.sendClickStartedAt)} /{" "}
                              ...
                              <dd>
                                ...
                                {formatDateTime(
                                  webView.manualDeliveryResolvedAt,
                                )}
                              </dd>
                            ...
                            {webView.transitionHistory.length > 0 ? (
                              <div className="mt-3 rounded border bg-[var(--surface)] p-2 text-xs">
                                <p className="font-medium">
                                  Histórico de transições
                                </p>
                                <ol className="mt-1 grid gap-1">
                                  {webView.transitionHistory.map(
                                    (entry, index) => (
                                      <li key={`${entry.at}-${index}`}>
                                        {entry.from} → {entry.to} /{" "}
                                        {formatDateTime(entry.at)} / ator{" "}
                                        {entry.by}
                                        {entry.reason
                                          ? ` / ${entry.reason}`
                                          : ""}
                            ...
                            <div className="mt-3 grid gap-2">
                              <p className="text-xs font-medium">
                                Comandos locais para copiar — não executados
                                pelo dashboard:
                              </p>
                            ...
                            <div className="mt-4 grid gap-3 border-t border-amber-200 pt-3">
                              ...
                              {canAuthorize ? (
                                <form
                                  ...
                                  className="grid gap-2 rounded border bg-[var(--surface)] p-2 text-xs"
                                >
                                  <input
                                    type="hidden"
                                    name="publicationId"
                      ...
                      <td className="sticky right-0 z-10 min-w-[300px] border-l bg-[var(--surface)] px-4 py-3 align-top">
                        {deliveryUncertain ? (
                          <div className="grid gap-2 rounded border border-amber-300 bg-[var(--warning-subtle)] p-3">
                            <p>
                              É possível que a mensagem tenha sido enviada.
                              Verifique o grupo antes de tomar qualquer ação.
                            </p>
                            <form
                          ...
                          <div className="grid gap-2 rounded border border-red-200 bg-[var(--danger-subtle)] p-3">
                            <p>
                              A entrega foi marcada como não realizada. Retry
                              continua bloqueado.
                            </p>
                            <form
                            ...
                            >
                              ...
                              <label className="flex items-start gap-2">
                                ...
                                />
                                <span>
                                  Autorizo explicitamente uma única nova
                                  tentativa.
                                </span>
                              </label>
                              <Button type="submit" variant="outline">
