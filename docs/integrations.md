...
Discovery is configured in `/integracoes/mercado-livre`. Category IDs are selected by the operator; the system does not hardcode categories or import the complete category tree. The worker deduplicates item IDs from highlights, fetches details and official prices, normalizes only available facts and passes candidates into `ingestOffer`.
