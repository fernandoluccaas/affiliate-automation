...
Phase 5F adds an operations plane without changing that dispatch boundary. Docker Desktop owns PostgreSQL and Redis; a singleton PowerShell supervisor owns the localhost production dashboard and one Redis-elected continuous worker. Local `.local` files contain only sanitized process/log/backup state. Health, status, audit, and `/operacoes` are read-only. WhatsApp browser commands remain an attended CLI plane and are absent from supervisor, process host, Task Scheduler, dashboard actions, and the continuous worker.
