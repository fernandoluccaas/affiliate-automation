...
Preflight is the final safe check while `WHATSAPP_WEB_DRY_RUN=true`. It waits for a stable media preview with no upload in progress, fills only the current media editor caption, and verifies that the complete immutable snapshot remains after a DOM stabilization cycle. If WhatsApp recreates the caption input, one bounded refill is allowed; missing or divergent content fails as `CAPTION_INPUT_RECREATED`, `CAPTION_CONTENT_LOST` or `CAPTION_CONTENT_MISMATCH`. Preflight then revalidates the exact open group, exactly one affiliate URL, the title snippet, absence of a visible upload error, and exactly one visible/enabled send trigger scoped to the media editor. It is structurally separate from `clickSendTrigger`, never sends, and clears the draft before closing the browser and releasing the Redis lock.
