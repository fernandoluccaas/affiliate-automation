...
9. Run `npm run whatsapp:web:inspect-draft -- --publication-id <id> --hold-ms 20000`, visually verify the correct group, image, complete caption and affiliate link, then answer `s` only if all are visible. The command never sends and must clear the draft.
10. Run `npm run whatsapp:web:config-check -- --publication-id <id>`. While `WHATSAPP_WEB_DRY_RUN=true`, it must report `realSendEligible=false`, `blockingReason=WHATSAPP_WEB_REAL_SEND_DISABLED_BY_DRY_RUN` and `browserOpened=false`, without acquiring media, profile lock or Chromium.
