...
function trackingStrategyForOffer(input: OfferFormValues) {
  return input.trackingStrategy ?? (input.marketplace === "MERCADO_LIVRE" ? "DIRECT_AFFILIATE_LINK" : "INTERNAL_REDIRECT");
}
...
export async function ingestOfferInTransaction(
  ...
  const shouldCreateInternalAffiliateLink =
    input.affiliateUrl && trackingStrategy === "INTERNAL_REDIRECT";
  ...
  if (shouldCreateInternalAffiliateLink && !existingAffiliateLink && affiliateLinkSlug && input.affiliateUrl) {
